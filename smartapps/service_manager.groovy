...
def keypadMsgHandler(evt) {
    log.trace("--- manager.keyppadMsgHandler evt=${evt}")
    def name        = evt.name
    def value       = evt.value
    def device      = evt.device
    def description = evt.description
    log.trace("--- manager.keyppadMsgHandler evt name=${name} value=${value} device=${device} description=${description}")
    def d   = evt.device
    def dni = d.getDeviceNetworkId()
    log.trace("--- manager.keyppadMsgHandler evt device.dni=${dni}")
    
    def keypad = getChildDevice("${dni}:keypad")
    if(keypad)
      keypad.sendEvent(name: "keypadMsg", value: value)
}

def chimeHandler(evt) {
    log.trace("--- manager.chimeHandler evt=${evt}")
    def name        = evt.name
    def value       = evt.value
    def device      = evt.device
    def description = evt.description
    log.trace("--- manager.chimeHandler evt name=${name} value=${value} device=${device} description=${description}")
}
...
def findAlarmDecoders() {
    def ad_list  = []
    def ad       = null
    def children = getAllChildDevices()
    children.each{
      def ad_flag = it.getDataValue("isAlarmDecoder")
      if(ad_flag!=null && ad_flag==true)
         ad_list.add(it)
    }    
    return ad_list
}
    ...
    selected_devices.each { dni ->
        ...
        if (!d) {
            ...
            if (newDevice!=null) {
                ...
                **/
                def ad_label="Alarm Decoder ${state.mac}"
                ...
                try{
                     d = addChildDevice("alarmdecoder",                         // namespace
                                        "Alarm Decoder Network Appliance",      // typename
                                        ad_dni,                                 // device network id
                                        newDevice?.value.hub,                   // hub id
                                        [isComponent: false,
                                         componentName: "AlarmDecoder" , 
                                         componentLabel: ad_label, 
                                         completedSetup: true, 
                                         data:[isAlarmDecoder: true, urn: state.urn, mac: state.mac, dni: ad_dni, ip: state.ip, port: state.port, 
...
def subscribeAll() {
    ...
    devices.each { d-> 
       ...
       // through alarm decoder. They will have no effect on physical or RF zones, just emulated virtual zones.
           ...
           subscribe(d, "keypadMsg",         keypadMsgHandler,         [filterEvents: false])
           subscribe(d, "chime",             chimeHandler,             [filterEvents: false])
    } // devices
}
...
def deleteSibling(dni) {
    def d     = getChildDevice(dni)
    def label = d.getLabel()
    def success=true
    try {
        deleteChildDevice(dni)
    } catch (Exception e) {
         success=false
         Alert("Error: Problem deleting device ${label}: ${e.message}")
    }
}

def deleteSiblingDevices(dni) {

    log.trace("---deleteSiblingDevices: ${dni}")
    ...
    children.each{
       if( it.deviceNetworkId.startsWith("${dni}") && ! it.deviceNetworkid.endsWith(":alarmdecoder") ) 
          success=deleteSibling(dni)
    }
...
def uninstallAlarmDecoder(dni) {
    ...
    // First delete sibling devices zonecontrols zonesensors, relaysensors, keypad, etc.
    def success=deleteSiblingDevices(dni)
         
    def d = getChildDevice(dni)
    
    //TODO: Should do something about devices/selectedDevices to unselect this AD.
        ...
        {  
            ...
            try{
                  zone_control = addChildDevice("alarmdecoder",         // namespace
                                                ...
                                                data:[isZoneControl : true, ad_dni: dni, mac: state.mac]]
                                                )
                  created=true
