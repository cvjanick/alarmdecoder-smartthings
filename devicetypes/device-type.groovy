...
preferences {
     ...
     ***/
     section(name: "ChildDevicePrefs", title: "Child Device Preferences") {
        input("create_keypad",   "bool", title: "Create Keypad Device", 
              description: "Create a child keypad device that looks similar to the AlarmDecoder WebApp keypad",
              required: false, defaultValue: false)
        input("show_keypad",   "bool", title: "Show Keypad Device", 
              description: "If Create keypadis true, show the child keypad device that looks similar to the AlarmDecoder WebApp keypad",
              required: false, defaultValue: false)
        input("use_virtual_zones",   "bool", title: "Create Virtual Zone Controls (ADEMCO ONLY)", 
              ...
              required: false, defaultValue: false)
        input("show_virtual_zones",   "bool", title: "Show Virtual Zone Control Devices", 
              description: "If Create keypadis true, show the child virtual zone control devices configured in AD and the AD WebApp",
              required: false, defaultValue: false)
...
def findKeypadDevice() {
  ...
  devices.each{ d ->
     if( d.getTypeName()=="Alarm Decoder Keypad Component")
        kpd=d
   }
   return kpd
}
def addKeypadDevice() {
    ...
    if(d!=null) {
      ...
      def kpdni = d.getDeviceNetworkId()
      def alist=d.getSupportedAttributes()
      log.debug"removing device=${d}, dni=${kpdni}, keypad attributes=${alist}"
      deleteChildDevice(kpdni)
    }
    // Now add Device
    try {
    def cd = addChildDevice(
                "alarmdecoder",
				"Alarm Decoder Keypad Component",
				"${urn}:Keypad",
				null, // hubid
				[completedSetup: true, 
                 isComponent: visible, 
                 label: "AlarmDecoder Keypad Component",
                 componentName: "KeypadComponent1", 
                 componentLabel: "KeypadComponent1"
                 ])
     } catch(Exception e) {
...
def send_st_settings_request() {
    log.trace("--- send_st_settings_request ")
    def urn    = getDataValue("urn")
    //def host   = urn.substring(0, urn.indexOf(":"))
    //def path   = "/api/vi/smartthings/set?apikey=${apikey}"
    // def host   = "requestb.in"
    def apikey   = _get_api_key()
    def apipath  = "?apikey=${apikey}"
    def testhost = "requestb.in"
    def testpath = "/1aydqcn1${apipath}"

    def body = "{  'access_token' : \"${state.access_token}\" 'endpoint_url' : \"${state.endpoint_url}\" }"
    def request = hub_https_post(testhost, testpath, body)
