...
def findKeypadDevice() {
    ...
    devices.each{ d ->
       ...
       log.debug "findKeypadDevice child_dni=${d_dni}  search_dni=${dni}"
       if(d_dni==dni)
         kpd=d
    }
...
def addKeypadDevice() {
    ...
    //def ad      = device.deviceNetworkId()
    def ad_dni  = device.deviceNetworkId
    def d       = findKeypadDevice()
    def kp_dni  = "${ad_dni}:keypad"
    
    log.debug "addKeypadDevice keypad search d=${d}"
    
    if(d==null) {
      // Device is null so add it
      //
      log.debug "adding Keypad Device..."
      def cd = null
      try {
        cd = addChildDevice(
                "alarmdecoder",
            ...
            parent.Alert("Error adding keypad device - make sure VirtualKeypad.groovy is installed in the IDE, ${e.message}" )
     }
...
def deleteFunctionKeyDevice(name) {
      ...
      if(d!=null)
        try {
              ...
              log.error "Error deleting child device ${name}: ${e.message}"
              Alert( "Error deleting child device ${name}: ${e.message}" )
        }
        
}

def findFunctionKeyDevice(name) {
  def dni = "${device.deviceNetworkId}:function:${name}"
  def kpd=null
  def devices=getChildDevices()
  devices.each{ d ->
    ...
    try {
        cd = addChildDevice(
				...
				[completedSetup: true, 
                 ...
                 data: [isFunctionKey: true, value: val, label: "${label}", delay: delay, dni: "${dni}:function:${name}", urn: urn]
                 ])
     } catch(Exception e) {
            log.debug "addFunctionKeyDevice error=${e.message}"
            parent.Alert("Error adding function key device - make sure VirtualFunctionKey.groovy is installed in the IDE, ${e.message}" )
     }
